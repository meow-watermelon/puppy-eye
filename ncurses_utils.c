...
void construct_window_layout(WINDOW *input_window) {
    ...
    */
    ...
    mvwhline(input_window, 6, 1, ACS_HLINE, COLS - 2);

    wattron(input_window, A_STANDOUT);
    mvwprintw(input_window, 7, 1, memory_metrics_banner);
    wattroff(input_window, A_STANDOUT);
    wattron(input_window, A_BOLD);
    mvwprintw(input_window, 9, 1, memory_usage_layout);
    mvwprintw(input_window, 10, 1, swap_usage_layout);
    mvwprintw(input_window, 11, 1, page_faults_layout);
    wattroff(input_window, A_BOLD);
    mvwhline(input_window, 13, 1, ACS_HLINE, COLS - 2);

    wattron(input_window, A_STANDOUT);
    mvwprintw(input_window, 14, 1, network_metrics_banner);
    wattroff(input_window, A_STANDOUT);
    wattron(input_window, A_BOLD);
    mvwprintw(input_window, 16, 1, arp_cache_entries_layout);
    mvwprintw(input_window, 18, 1, interface_io_banner_layout);
    mvwprintw(input_window, 19, 1, interface_layout);
    wattroff(input_window, A_BOLD);
